...
(function (ummath, ummesh, ummaterial) {
	...
	UMNode.prototype.update = function () {
		...
		var global = this.global_transform;
		var parent_global;
		if (this.parent) {
			parent_global = this.parent.global_transform;
		} else {
			parent_global = this.global_transform;
		}
		...
		var middle = (start.add(end)).scale(0.5);
		...
		dst_octahedron[1] = middle.add(dir.cross(global_y).scale(0.1 * len));
		dst_octahedron[2] = middle.add(dir.cross(global_z).scale(0.1 * len));
		dst_octahedron[3] = start;
		dst_octahedron[4] = middle.add(dir.cross(global_y).scale(-0.1 * len));
		dst_octahedron[5] = middle.add(dir.cross(global_z).scale(-0.1 * len));
		...
		if (this.mesh.material_list.length === 0) {
			...
			meshmat.set_diffuse(0.5, 0.5, 0.5, 0.5);
