...
(function (ummath) {
	...
	UMCamera = function (gl, w, h) {
		...
		this.near = 0.05;
		this.far = 5000.0;
		this.position = new ummath.UMVec3d(0, 0.0, 30.0);
		this.target = new ummath.UMVec3d(0, 0.0, 0);
	...
	UMCamera.prototype.update = function () {
		...
		for (i = 0; i < 4; i = i + 1) {
			for (k = 0; k < 4; k = k + 1) {
				this.view_matrix_.m[i][k] = window.viewmat[i * 4 + k];
			}
		}
	...
	UMCamera.prototype.rotate = function (mx, my) {
		var cx,
			radius,
			rot,
			oldx = this.rotation.xyz[0],
			oldy = this.rotation.xyz[1];

		this.rotation.xyz[1] += ummath.um_to_radian(mx * 0.279);
		this.rotation.xyz[0] += ummath.um_to_radian(my * 0.279);
		...
		if (cx < ummath.EPSILON) {
			this.rotation.xyz[0] = oldx;
			this.rotation.xyz[1] = oldy;
			return;
		}
